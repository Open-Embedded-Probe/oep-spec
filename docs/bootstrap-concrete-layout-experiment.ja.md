...
このcached responseとsilent busyの組合せは、一件ずつ操作するhostでも暗黙対応だけでは不十分な例になる。[明示correlationと暗黙対応の比較](implicit-correlation-comparison.ja.md)では、初期bootstrapと通常request/resultの両方で明示correlationを維持する方向とした。仮16 bit幅を採用したことは意味しない。
