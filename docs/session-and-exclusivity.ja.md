...
- **排他は session_id で行う。** OS やライブラリの排他（Windows の COM ポート、libusb の claim、pyserial の
  ...
  protocol はそれに頼らない。
- 経路ごとに別のセッションを持たせ、probe が送り主ごとにキューで順に処理する形は v1 では扱わない（排他は経路ではなく
  session_id で決まる）。
...
- **session_id は host が乱数で選ぶ**（u32）。probe に乱数も不揮発のカウンタも要求しない。
- **session_id はフレームのヘッダに置き、「session_id あり」のフラグで省けるようにする**（2026-09-24 の合意）。
  probe は endpoint の 1 か所ですべての要求を確かめられ、操作ごとの確認漏れが起きない。ロックなしで使える読むだけの
  要求は省く。負担の実測は [experiments/session-id-cost](../experiments/session-id-cost/README.ja.md)（USB では
  1 % 未満、UART 115200 bps の 64 byte フレームや小さなポーリングで約 7 %）。
...
- この保証が成り立つために、**状態を変える操作はすべて session_id を要する。** ロックなしで使えるのは読むだけの
  操作（confirm、list、describe、ロックの状態、**コンソールの read と marks**）に限る。
  - コンソールの read / marks をロックなしにしても混ざらない（2026-09-24 の合意）。読み出しは位置を指定するだけで
    バッファを消費せず、probe は読み手ごとの状態を持たない（どこまで読んだかは host が覚える）。状態も変えないので、
    「session_id が変わっていなければ誰も操作していない」も崩れない。
...
開閉でリセットしない、DTR に頼らない、セッションを失っても状態を戻さない。具体的なやり方は
[probe 開発ガイド](probe-development-guide.ja.md) に書く。
...
1. （決定: ヘッダ、u32、フラグで省略可。上記）フラグとフィールドのビット配置は wire format と一緒に決める。
2. リース時間の範囲と既定値。
3. （決定: コンソールの read / marks はロックなしで許す。上記）
