...
これにより、独自機能の定義を理解する別の実装者が、元の製品固有の通信経路を再利用せず、OEP上で対応するhostまたはprobeを実装できる状態を目指す。独自機能であることは許容するが、OEPを迂回する通信は許容しない。

この原則は、独自機能の識別方法、定義の公開範囲、拡張形式またはdata表現をこの段階で決めるものではない。
