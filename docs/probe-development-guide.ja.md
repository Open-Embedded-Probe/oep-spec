...
- **RVSWD のアイドル中の線の向きは target の性質で、CH32X035 と CH32L103 で逆になる**（2026-09-24、OEP 側での観察。
  ...
  - つまり L103 は、両方 high で 36 ms 休ませても LinkE とは通信を続けられる。OEP の probe で両方 high にすると 20 ms で
    ...
    - 手がかり（未確認）: 両方 high で休ませる一時ビルドの RP2350 につながった L103 は、休止中の自分のピンを
      「SWDIO = low、SWCLK = high」と読んだ（スタート条件の途中にあたる）。probe は両方 high に駆動しているはずで、
      食い違っている。L103 は resume の確認が取れないので、止めて RAM を読む観察を繰り返せず、ここで止めた。
  - 同じ L103 で WCH-Link と OEP の probe の休ませ方をそろえられれば、target ごとの設定は要らなくなる見込みがある。
  - いまは probe のビルドの設定（治具のプロファイル）で持っている。1 台の probe が両方の target を相手にするようになったら、
    host が attach で指定する形を考える。
