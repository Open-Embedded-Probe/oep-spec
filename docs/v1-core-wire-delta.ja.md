...
1. （試作: subscribe の引数、上記）まとめて送る条件。
2. 出来事の kind の割り当て（fn 0 のハートビート以外）。候補: fixture の GPIO の割り込み（エッジ、時刻付き）、target の
   停止やリセット、キャプチャの完了、ロックの期限切れの予告。インターフェースごとに定める。
3. ロックの持ち主以外（ロックなしで読める、コンソールなど）に購読を許すか。
4. 時刻の単位と基準（試作は probe の起動からの µs、u32 で約 71 分で一周）。
