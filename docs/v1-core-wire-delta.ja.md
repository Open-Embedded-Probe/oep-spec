...
- `seq` は fn ごとのフレームの通し番号（一周する）。抜けがあればフレームが失われた（UART の経路など）。
...
| op | 名前 | 要求 | 応答 | ロック |
|---:|---|---|---|---|
| 0x30 | subscribe | fn(u16) | — | 必要 |
| 0x32 | unsubscribe | fn(u16) | — | 必要 |

- **購読はロックの持ち主だけができ、ロックと一緒に終わる**（end、または期限切れ）。host が消えても、期限が来れば probe は
  送るのをやめる。購読している host は keepalive などでロックを保つ。
...
**そのほか**: 購読していない間は何も来ない。化けと seq の抜けは全条件で 0。流量が少ないとフレームが細かくなる（50 kB/s で
1 フレーム約 1 バイト）ので、**何バイトたまるか、何 ms 経つまで待って送るかの指定が要る**（未実装）。通知の利点は速さでは
なく、応答の待ち時間と、host が位置を管理しなくてよいこと。

### 未決

1. まとめて送る条件（何バイト / 何 ms）を subscribe の引数にするか、インターフェースの設定にするか。
2. 出来事（0x05）の形と種類。候補: fixture の GPIO の割り込み（エッジ、時刻付き）、probe のハートビート（生きていること、
   boot_id）、target の停止やリセット、キャプチャの完了、ロックの期限切れの予告。
3. ロックの持ち主以外（ロックなしで読める、コンソールなど）に購読を許すか。
