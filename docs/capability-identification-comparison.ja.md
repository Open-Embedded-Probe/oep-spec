...
3. **版**（2026-09-26 に改め、[v1 wire](v1-core-wire-delta.ja.md) §0 に一本化）: revision を上げるのは固定部分の意味か
   長さを変えるときだけ。任意の TLV、任意の op、任意の event の追加では revision を変えない（上げると、知らない revision を
   使わない古い host がインターフェースごと使えなくなる）。名前を変えるのは意味が変わるときだけ。
