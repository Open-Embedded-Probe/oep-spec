...
|---|---:|---:|---:|---:|
| B: compact core + HID wrapper | 502 byte | 17 byte | 16 byte | 1 |
| C: fixed record | 404 byte | 10 byte | 9 byte | 2 |

static RAMには共有report bufferと1 byteの測定用sinkを含む。候補Bはbinding非依存の10 byte request / 14 byte responseを扱うcore handlerと、16 byte HID report wrapperへ分離した。AVRではcore handlerが184 byte、HID wrapperが68 byteで、候補Cの専用handlerは152 byteだった。候補Bはcompatible、incompatible、unknown operationおよびidentityを確認できるmalformed lengthのrejectionを区別する。HIDだけを比較すると候補Bはwrapper分だけ大きいが、UART等は同じcore handlerを共有できる。
...
|---|---:|---:|---:|
| avr-gcc 7.3.0 ELF | 502 byte | 526 byte | +24 byte Flash |
| RISC-V GCC 14.3.0 text | 604 byte | 628 byte | +24 byte text |
...
xPack RISC-V GCC 14.3.0、`-Os`で、候補Bはtext 604 byte / RAM section 17 byte、候補Cはtext 548 byte / RAM section 13 byteだった。候補Bのcore handlerは290 byte、HID wrapperは62 byte、候補C専用handlerは252 byteだった。startup、rv003usb、interruptおよびUSB endpoint stateは含まない。
...
|---|---:|---:|---:|---:|
| B | 2,528 byte | 112 byte | 16 byte | 1 |
...
成立するHID構成同士では、候補Cの削減はFlash 64 byte / RAM 4 byteだった。候補Bはunknown operationと、identityを確認できる長さ不正をcorrelation付きresponseで拒否する処理も含む。core handlerはHID wrapperから独立しており、UART等でも共有できる。lifecycleは未取得responseが次のSET_REPORTで上書きされることを防ぎ、正しい長さのGET_REPORT開始後にだけbufferを次のsetupで再利用可能にする。SET_REPORT受信途中またはGET_REPORT送信途中に次のSET_REPORT/GET_REPORT setupが始まった場合は、前のcontrol transferを終了して新しい要求を評価する。4状態と6操作の全24組合せをhost testで確認した。
...
このHID候補では、GET_REPORTを一度開始または完了したことだけをcorrelationのretire根拠にできない。実験上cacheが到着不能になるのは、次の正しいSET_REPORTがcacheを置換した場合、lifecycle reset、またはfirmware再初期化である。将来のhost bindingは、同じcached reportを複数回取得しても同じlogical resultを上位へ複数deliveryしない必要がある。
...
core parserが要求する10 byteと異なる0、7、8、9、15および32 byteのlogical messageもUARTで送った。8 byte未満ではOEP identity全体を確認できないため無応答とする。8 byte以上でrequest role、correlationおよびidentityを確認でき、response用capacityもある場合は、元のoperationとcorrelationを含む仮`malformed` responseを返す。いずれもbindingは一度だけdeliveryしてtransport ACKを返し、message長不一致をreceiver busy、transport破損またはbinding retryへ変換しない。

core parser単体ではmessage length 0から32 byteと、response size未満のcapacity 0から13 byteもguard byte付きで全走査した。identityを読めない長さまたは14 byteのresponseを格納できないcapacityでは書込みを行わず、responseを生成できる場合も指定buffer外を変更しない。

HID wrapperもeffective core lengthを独自に10 byteへ限定せず、固定report長とreport IDを確認した後は同じcore parserへ判断を委ねる。これによりidentityまで含む8または9 byteの短いcore requestはUARTと同じ`malformed` responseになり、8 byte未満または14 byteのreport payload capacityを超えるlengthは無応答になる。
