...
static void cobs_writer_init(
    struct cobs_writer *writer,
    uint8_t *output,
    size_t capacity)
{
...
static void cobs_writer_put(struct cobs_writer *writer, uint8_t byte)
{
    if (writer->failed) {
        return;
    }
...
static size_t cobs_writer_finish(struct cobs_writer *writer)
{
...
static size_t cobs_decode_in_place(uint8_t *data, size_t encoded_length)
{
    size_t read_index = 0;
    ...
    size_t wire_capacity)
{
    ...
    header[0] = (uint8_t)((type << 4) | sequence);
    header[1] = (uint8_t)payload_length;
    header[2] = (uint8_t)(payload_length >> 8);

    cobs_writer_init(&writer, wire, wire_capacity);
    for (index = 0; index < sizeof(header); ++index) {
        crc = crc16_update(crc, header[index]);
        cobs_writer_put(&writer, header[index]);
    }
    for (index = 0; index < payload_length; ++index) {
        crc = crc16_update(crc, payload[index]);
        cobs_writer_put(&writer, payload[index]);
    }
    cobs_writer_put(&writer, (uint8_t)crc);
    cobs_writer_put(&writer, (uint8_t)(crc >> 8));

    return cobs_writer_finish(&writer);
}
    ...
    struct oep_uart_frame_view *frame)
{
    ...
    raw_length = cobs_decode_in_place(
        decoder->encoded,
        decoder->encoded_length);
    decoder->encoded_length = 0;
    if (raw_length < OEP_UART_RAW_OVERHEAD) {
        return OEP_UART_DECODE_DROPPED;
    }
    ...
    payload_length = (uint16_t)decoder->encoded[1] |
        ((uint16_t)decoder->encoded[2] << 8);
    if (payload_length > OEP_UART_MAX_PAYLOAD ||
        raw_length != (size_t)payload_length + OEP_UART_RAW_OVERHEAD) {
        return OEP_UART_DECODE_DROPPED;
    }

    expected_crc = (uint16_t)decoder->encoded[raw_length - 2] |
        ((uint16_t)decoder->encoded[raw_length - 1] << 8);
    actual_crc = oep_uart_crc16(decoder->encoded, raw_length - 2);
    ...
    frame->payload = decoder->encoded + 3;
