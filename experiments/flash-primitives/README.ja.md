...
| （脚注）LinkE 側の読み戻しの確認が長いのは、ch32rv の読み出しが usbipd 越しで約 56 KB/s しか出ないことが主（ch32rv のレビュー、2026-09-24）。このベンチでの比較としては公平だが、ネイティブ USB の LinkE ならもっと速いはずなので、「OEP の方が LinkE より速い」とは読まない | | | |
...
頼らない）。X035 用のローダーがそのまま L103 に使えた（flash の手順が同じ）。

| L103 62 KB（`f5_v1.py --batch 8`） | 消去 + 書き込み | 読み戻しの確認 | 書き直したページ |
|---|---|---|---|
| 1 回目 / 2 回目 | 5.25 秒 / 5.16 秒 | 1.25 秒 / 1.20 秒 | 1 / 3（最終的に全面一致） |

- この治具は DMI がときどき化ける。最初は 248 回中 2〜4 回のローダーの実行が失敗した: resumereq が効かず開始位置の
  まま止まる（L103 は allresumeack も立てない）、止まったあとの DMI の線が落ちて読めない、ebreak でない場所
  （dpc 0x2）で止まる。
- probe の `runUntilHalt` に、v0 の halt / resume と同じ対策を入れた: 読み出しに失敗したら線を立て直す、止まった
  あと dpc を読む前に立て直す、dpc が開始位置のままなら resumereq を出し直す。X035 / V003 でも今までどおり通った。
- それでも残る化けは、host がページ単位で書き直す（失敗したページと読み戻しで違ったページを 2 回まで。v0 の host と
  同じ）。`f5_v1.py` に入れた。
- 実際のスケッチ（DmSeqTest）を書いてリセットし（flags 0x7、2 回目で確定）、コンソールはエコー、reset / restart の
