...
static uint8_t message[OEP_UNIFORM_HEADER_SIZE] = {
    OEP_HEADER_ROLE_FUNCTION_REQUEST, 1u,
    OEP_HEADER_SCOPE_OFFERED_FUNCTION, 0x34u, 0x12u, 0x01u, 0x10u,
};
...
int main(void)
{
    ...
    if (!oep_decode_uniform_header(message, sizeof(message), &view)) {
        return 1;
    }
